 ...
 */
public class PIMContact extends PIMEntity implements PIMDate {
    ...
    public void setFromString(String fmtStr) {
        ...
        Pattern pattern = Pattern.compile("^CONTACT ([^ ]+?) ([0-9]{2}/[0-9]{2}/[0-9]{4}) first-name:(.*?) last-name:(.*?) emaIl:(.*?)$");
        Matcher matcher = pattern.matcher(fmtStr);

        if (!matcher.matches()) {
            throw new IllegalArgumentException("参数 `fmtStr` 格式非法！（e.g. \"CONTACT normal 11/30/2018 first-name:Yang-hao last-name:LUO emaIl:keyboard-l@outlook.com\"");
        }
        ...
        this.email = matcher.group(5);
    }
    public String toString() {
        String res = "CONTACT ";
        res += this.getPriority() + " ";
        res += formatDate(this.date) + " first-name:";
